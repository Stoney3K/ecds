...
struct _ecds_dispatcher_t {
	...
	pthread_condattr_t dispatcher_cond;
};

static void _dispatcher_thread(ecds_dispatcher_t * disp)
{
	bool running = true;
	...
	while (running)
	{
		...
		while (ecds_queue_peek(disp->message_queue) == NULL) {
			pthread_cond_wait(&disp->dispatcher_cond, &disp->dispatcher_mutex);
		}

		for (ecds_list_item_t * iter = ecds_queue_dequeue_item(disp->message_queue); iter;)
		{
			/* Extract message and dequeue the next */
			ecds_message_t * msg = (ecds_message_t *)ecds_list_get_item(disp->message_queue, iter);
...
static void _dispatcher_dispose(ecds_dispatcher_t * disp)
{
	...
	pthread_join(disp->dispatcher_thread, (void **)&retval);
...
{
	char queue_name[80];
	ecds_dispatcher_t * ret = ecds_object_new(name, sizeof(ecds_dispatcher_t), ECDS_DISPATCHER);

	sprintf(queue_name, "%s-queue", name);
	ret->message_queue = ecds_queue_new();
	ecds_object_rename(OBJECT(ret->message_queue), queue_name);

	ecds_info("Constructing new dispatcher: %s", ecds_object_get_name(OBJECT(ret)));
}

void ecds_dispatcher_queue_message(ecds_dispatcher_t * disp, ecds_message_t * msg) 
{
	pthread_mutex_lock(&disp->dispatcher_mutex);
	ecds_queue_enqueue_item(disp->message_queue, ecds_list_create_item(OBJECT(msg)));
	pthread_mutex_unlock(&disp->dispatcher_mutex);
}
...
void ecds_dispatcher_subscribe(ecds_dispatcher_t * disp, unsigned int event_type, ecds_service_t * service)
{
	for (ecds_list_item_t * iter = ecds_list_first_item(disp->event_list); iter; iter = ecds_list_next_item(iter))
	{
		...
		if (evt->event_type == event_type)
		{
			...
			ecds_list_add_item(evt->service_list, OBJECT(service));
			ecds_info("Adding service %s for event ID %08X", ecds_object_get_name(OBJECT(service)), event_type);
		}
	}

	/* Event type was not found, add new */
	char event_name[128];
	sprintf(event_name, "%s-event-%8X", ecds_object_get_name(OBJECT(disp)), event_type);

	ecds_info("Adding new event ID %08X", event_type);
	ecds_dispatcher_event_t * evt = ecds_object_new(event_name, sizeof(ecds_dispatcher_event_t), ECDS_DISPATCHER_EVENT);
...
void ecds_dispatcher_dispatch_message(ecds_dispatcher_t * disp, ecds_message_t * msg)
{
	...
	{
		ecds_dispatcher_event_t * evt = (ecds_dispatcher_event_t *)ecds_list_get_item(disp->event_list, iter);
		if (evt->event_type == msg->event_type)
		{
			event = evt;
			break;
		}
	...
	if (event == NULL)
	{
		...
		char event_name[128];
		sprintf(event_name, "%s-event-%8X", ecds_object_get_name(OBJECT(disp)), msg->event_type);

		ecds_info("Received message with unknown event ID %08X, registering for future dispatch", msg->event_type);
		ecds_dispatcher_event_t * evt = ecds_object_new(event_name, sizeof(ecds_dispatcher_event_t), ECDS_DISPATCHER_EVENT);
		evt->service_list = ecds_list_new();
		return;
	}
